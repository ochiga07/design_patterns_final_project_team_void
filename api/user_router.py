...
def get_users(user_service: Annotated[UserService, Depends(get_user_service)]) -> list[UserResponseDto]:
    return user_service.get_all_users()

@user_router.get("/{user_id}", response_model=UserResponseDto)
def get_user(user_id: int, user_service: Annotated[UserService, Depends(get_user_service)]) -> UserResponseDto:
